...
custom BIOS. You still need a dummy kernel image to make Qemu happy.
Try copying /boot/vmlinuz to ~/dummy_os.elf.
...
- kAFL requires an -ip0 parameter to indicate the address range of the target
  ...
  extracted from the UEFI boot log. To get you started, the helper script uses
  a very large (invalid) range leading to potential non-determinisim in the
  coverage feedback.
...
$ cp Build/OvmfX64/RELEASE_GCC5/FV/OVMF.fd $way/$to/$go/bios.bin
```
...
```
$ ./qemu-4.0.0/x86_64-softmmu/qemu-system-x86_64 -enable-kvm --pflash bios.bin -net none -serial mon:stdio -kernel ~/dummy_os.elf
```

Build an EFI app and test if it is presented and loaded properly in the VM:

```
$ ./targets/uefi_ovmf_64/compile.sh app # build app and copy to fake_hda/harness.efi
$ FAKE_HDA=$PWD/targets/uefi_ovmf_64/fake_hda
$ qemu-system-x86_64 -pflash bios.bin -hda fat:rw:$FAKE_HDA -net none -serial mon:stdio -kernel ~/dummy_os.elf
```
