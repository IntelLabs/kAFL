...
static const char *device_name = "vuln_json";
...
static int vuln_write(dev_t dev, uio_t uio, int flags){
	...
	int res = jsmn_parse(&parser, buffer, iosize, tokens, 5);
	if(res >= 2){
		if(tokens[0].type == JSMN_STRING){
			int json_len = tokens[0].end - tokens[0].start;
			if(json_len > 0 && buffer[tokens[0].start+0] == 'K'){
				if(json_len > 1 && buffer[tokens[0].start+1] == 'A'){
					if(json_len > 2 && buffer[tokens[0].start+2] == 'F'){
						if(json_len > 3 && buffer[tokens[0].start+3] == 'L'){
						  PANIC;
						}
					}
				}
